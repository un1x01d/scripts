...
# Create Prometheus counters for each port with labels and the total hits with labels
hit_counters = {
    port: Counter(
        f'port_{port}_hits_total',
        f'Number of hits for port {port}',
        labelnames=['service_name']
    ) for port in port_service_names.keys()
}
total_hits_counter = Counter(
    'total_hits_total',
    'Total number of hits across all ports',
    labelnames=['port']
)
...
def simulate_service(port):
    ...
    while True:
        try:
            ...
            # Increment the hit counter for this port with service label
            hit_counters[port].labels(service_name=service_name).inc()
            # Increment the total hit counter with port label
            total_hits_counter.labels(port=str(port)).inc()
...
def main():
    ...
    start_http_server(10001)  # Port for Prometheus to scrape metrics
